...
class FraudDeepModelTrainer:
    ...
    def load_data(self):
        """Load fraud and credit card datasets."""
    ...
    def select_model(self, input_shape):
        """Initialize deep learning model based on user selection."""
        if self.model_type == 'mlp':
            self.model = Sequential([
                Input(shape=(input_shape,)),  # Corrected input shape
            ...
            self.model = Sequential([
                Input(shape=(input_shape, 1)),  # Corrected input shape
    ...
    def train_and_evaluate(self, X_train, X_test, y_train, y_test):
        ...
        # Reshape data if CNN or LSTM
        if self.model_type in ['cnn', 'lstm']:
            X_train = X_train.reshape(-1, X_train.shape[1], 1)
            X_test = X_test.reshape(-1, X_test.shape[1], 1)

        self.model.fit(X_train, y_train, epochs=10, batch_size=32, validation_data=(X_test, y_test), verbose=1)
        y_pred = (self.model.predict(X_test) > 0.5).astype('int32')

        report = classification_report(y_test, y_pred, zero_division=1)  # Fix undefined precision warning
        print(report)
        return report
    ...
    def save_model(self):
        """Save trained model in MLflow and Pickle format."""
...
if __name__ == "__main__":
    ...
    OUTPUT_PATH = "/home/nahomnadew/Desktop/10x/week8/Adey_Inoviation_Inc/Models/mlp_deep_model"
